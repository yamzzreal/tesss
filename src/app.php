...
<?php elseif($page==='admin'): $u=requireAdmin(); $products=db()->query("SELECT id, title, description, price, category, image, status, created_at FROM products ORDER BY id DESC")->fetchAll(); $tops=db()->query("SELECT t.*,u.username FROM topups t JOIN users u ON u.id=t.user_id WHERE t.status='pending' ORDER BY t.id DESC")->fetchAll(); ?>

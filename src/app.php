...
if($action==='buy'){
    ...
    db()->beginTransaction();
    try{
        $s=db()->prepare("SELECT * FROM products WHERE id=? FOR UPDATE"); $s->execute([$id]); $p=$s->fetch();
        if(!$p || $p['status']!=='ready') throw new Exception("Produk tidak tersedia.");
...
<?php elseif($page==='account'): $u=requireLogin(); ?>
...
<?php elseif($page==='transactions'): $u=requireLogin(); $s=db()->prepare("SELECT t.*,p.title FROM transactions t LEFT JOIN products p ON p.id=t.product_id WHERE t.user_id=? ORDER BY t.id DESC");$s->execute([$u['id']]); ?>
...
<?php elseif($page==='admin'): $u=requireAdmin(); $products=db()->query("SELECT * FROM products ORDER BY id DESC")->fetchAll(); $tops=db()->query("SELECT t.*,u.username FROM topups t JOIN users u ON u.id=t.user_id WHERE t.status='pending' ORDER BY t.id DESC")->fetchAll(); ?>
...
<section class="panel"><h2>Produk</h2><?php foreach($products as $p): ?><div class="row"><span><?=h($p['title'])?></span><span><?=money($p['price'])?> · <?=h($p['status'])?></span></div><?php endforeach; ?></section>
<?php else: $products=db()->query("SELECT * FROM products ORDER BY id DESC")->fetchAll(); ?>
...
<div class="grid"><?php foreach($products as $p): ?><article class="card"><?php if($p['image']): ?><img src="<?=h($p['image'])?>" alt=""><?php endif; ?><small><?=h($p['category'])?></small><h3><?=h($p['title'])?></h3><p><?=h($p['description'])?></p><strong><?=money($p['price'])?></strong><?php if($p['status']==='ready'): ?><form method="post"><input type="hidden" name="action" value="buy"><input type="hidden" name="product_id" value="<?=$p['id']?>"><button>Beli dengan Saldo</button></form><?php else: ?><button disabled>SOLD</button><?php endif; ?></article><?php endforeach; ?></div>
<?php endif; ?>
</main></body></html>

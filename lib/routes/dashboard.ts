...
export default async function handler(req:VercelRequest,res:VercelResponse){
 ...
 try{
  const [today,yesterday,all,days,merchant]=await Promise.all([
   ...
   sql`SELECT to_char(gs.day_date,'YYYY-MM-DD') AS day,COALESCE(SUM(t.amount) FILTER(WHERE t.status='paid'),0)::bigint AS gross,COUNT(t.id)::int AS transactions FROM generate_series(CURRENT_DATE - INTERVAL '6 days',CURRENT_DATE,INTERVAL '1 day') AS gs(day_date) LEFT JOIN transactions t ON t.merchant_id=${s.userId} AND t.created_at::date=gs.day_date::date GROUP BY gs.day_date ORDER BY gs.day_date`,
   sql`SELECT id,name,email,qris_name,qris_city,active,created_at,api_key FROM merchants WHERE id=${s.userId} LIMIT 1`
  ]);
